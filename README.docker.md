...
The container is automatically built and published to `ghcr.io/slmingol/conjakeons-plus:latest` on every push to main via GitHub Actions.
...
```bash
docker build -t conjakeons-plus:latest .
```

### Run production container
```bash
docker run -d -p 3000:80 --name conjakeons-plus conjakeons-plus:latest
```

### Build development image
```bash
docker build -f Dockerfile.dev -t conjakeons-plus:dev .
```

### Run development container
```bash
docker run -d -p 3000:3000 -v $(pwd):/app -v /app/node_modules --name conjakeons-plus-dev conjakeons-plus:dev
```
...
Check logs:
```bash
docker-compose logs conjakeons-plus
```
